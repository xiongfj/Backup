...
/*
* 使用 @Html.Pager 或 @Ajax.Pager( 时必须调用这两个函数的一个!!


* 当需要使用页索引输入或下拉框、
调用Javascript API或使用Ajax分页模式时，
必须用Html.RegisterMvcPagerScriptResource()或
Ajax.LoadMvcPagerScript()方法注册或
动态加载MvcPager客户端jQuery插件，
并在插件注册之前引入jQuery脚本库。
