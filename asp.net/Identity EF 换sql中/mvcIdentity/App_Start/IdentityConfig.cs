...
namespace mvcIdentity
{
    ...
    {
        public MyApplicationUserManager(IUserStore<MyApplicationUser> store)
            : base(store)
        {
        }
        ...
        {
            // 传递什么进去!?
            // UserManager 内部会回调 MyUserStore
            var manager = new MyApplicationUserManager(new MyUserStore<MyApplicationUser>(/*context.Get<ApplicationDbContext>()*/));

            // 配置用户名的验证逻辑
            manager.UserValidator = new UserValidator<MyApplicationUser>(manager)
            {
                AllowOnlyAlphanumericUserNames = false,
                RequireUniqueEmail = true
            };
            ...
            // 注册双重身份验证提供程序。此应用程序使用手机和电子邮件作为接收用于验证用户的代码的一个步骤
            // 你可以编写自己的提供程序并将其插入到此处。
            manager.RegisterTwoFactorProvider("电话代码", new PhoneNumberTokenProvider<MyApplicationUser>
            {
                MessageFormat = "你的安全代码是 {0}"
            });
            manager.RegisterTwoFactorProvider("电子邮件代码", new EmailTokenProvider<MyApplicationUser>
            {
                Subject = "安全代码",
                BodyFormat = "你的安全代码是 {0}"
            });
            ...
            {
                manager.UserTokenProvider =
                    new DataProtectorTokenProvider<MyApplicationUser>(dataProtectionProvider.Create("ASP.NET Identity"));
            }
            return manager;
        }
    }
