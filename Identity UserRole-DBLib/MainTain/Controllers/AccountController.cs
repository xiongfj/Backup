...
namespace MainTain.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            if (!ModelState.IsValid || !Validating(model.Code))
            {
                return View(model);
            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
					...
					if (!rolesForUser1.Contains("普通用户"))
					{
		...
		{
			MainTain.Function.ValidateCode vCode = new MainTain.Function.ValidateCode();
		...
		/// <returns></returns>
		bool Validating(string code)
		{
			if (code != null && Session["ValidateCode"].ToString() != code.Trim())
			{
